...
| Full outgoing attachment content and delivery | Generated files or paths do not prove what reached the user | [Runtime and alternate-surface follow-up](host-evidence-boundaries-2026-09-10.md) traces the actual sent projection, pre-send changes and lossy transcript mirrors. Preserve confirmed text; full per-item settled attribution remains narrower than the existing intent/session APIs |
| Hermes audio provenance | A successful transcript can look exactly like user-typed quoted text, so the adapter cannot safely infer audio origin | The pinned [voice enrichment](https://github.com/NousResearch/hermes-agent/blob/2237be355906fbe6065ce1815711eee52b2d646e/gateway/run_inbound.py#L1935) removes the distinguishing wrapper. The [follow-up](host-evidence-boundaries-2026-09-10.md) now preserves already available row identity/time and distinguishes the pre-STT hook from successful clip evidence; do not label every quotation as audio |
...
The later [host-boundary investigation](host-evidence-boundaries-2026-09-10.md) adds supported preprocessing capture and no-model integration evidence. It does not upgrade this eight-request run into a full native model loop.
